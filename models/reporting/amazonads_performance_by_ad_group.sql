{{ config (
    alias = target.database + '_amazonads_performance_by_ad_group'
)}}
...
{%- set stg_fields = adapter.get_columns_in_relation(ref('_stg_amazonads_ad_groups_insights'))
                    |map(attribute="name")
...
WITH 
    ...
    (SELECT 
        ...
        {%- endfor %}
    FROM {{ ref('_stg_amazonads_ad_groups_insights') }}
    {%- if var('currency') != 'USD' %}
